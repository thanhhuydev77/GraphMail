codeunit 70001 "FSO Email - Graph API Helper"
{
    Permissions = tabledata "Email Inbox"=ri,
        tabledata "FSO Email - Graph API Account"=rimd;

    var AccountNotFoundErr: Label 'We could not find the account. Typically, this is because the account has been deleted.';
    ...
    begin
        ...
        if EmailGraphAPIAccount.FindSet()then repeat Accounts."Account Id":=EmailGraphAPIAccount.Id;
    ...
    procedure DeleteAccount(AccountId: Guid): Boolean var
        OutlookAccount: Record "FSO Email - Graph API Account";
    begin
        if OutlookAccount.Get(AccountId)then if OutlookAccount.WritePermission()then exit(OutlookAccount.Delete());
        exit(false);
    end;
    procedure EmailMessageToJson(EmailMessage: Codeunit "Email Message"; Account: Record "FSO Email - Graph API Account"): JsonObject var
    ...
    begin
        ...
        exit(EmailMessageToJson(EmailMessage, EmailMessageJson))end;
    procedure EmailMessageToReplyJson(EmailMessage: Codeunit "Email Message"; ReplyBody: Text; AsHtml: Boolean): JsonObject var
    ...
    begin
        ...
        if AsHtml then EmailBody.add('contentType', 'html')
        else
    ...
    procedure EmailMessageToJson(EmailMessage: Codeunit "Email Message"): JsonObject var
        EmailMessageJson: JsonObject;
    begin
    ...
    local procedure EmailMessageToJson(EmailMessage: Codeunit "Email Message"; EmailMessageJson: JsonObject): JsonObject var
        EmailBody: JsonObject;
        MessageJson: JsonObject;
        MessageText: Text;
    begin
        if EmailMessage.IsBodyHTMLFormatted()then EmailBody.Add('contentType', 'HTML')
        else
            EmailBody.Add('contentType', 'text');
        ...
        EmailMessageJson.WriteTo(MessageText);
        if StrLen(MessageText) > MaximumRequestSizeInBytes()then MessageJson:=EmailMessageJson
        else
        begin
    ...
    local procedure GetEmailRecipients(EmailMessage: Codeunit "Email Message"; EmailRecipientType: enum "Email Recipient Type"): JsonArray var
    ...
    procedure GetAccessTokenByEmailAccount(EmailGraphAPIAccount: Record "FSO Email - Graph API Account")OauthHeader: Dictionary of[Text, SecretText]var
    ...
    procedure RemoveParenthesis(pInputTxt: Text): Text begin
    ...
    var
        ...
        MessageId: Text;
    begin
        if MessageJson.Contains('message')then SendMailSingleRequest(OauthHeader, MessageJson, FromEmail)
        else
        begin
            ...
            foreach Attachment in Attachments do if Attachment.AsObject().Contains('AttachmentItem')then UploadAttachment(OauthHeader, FromEmail, Attachment.AsObject(), MessageId)
                else
                    PostAttachment(OauthHeader, FromEmail, Attachment.AsObject(), MessageId);
            SendDraftMail(OauthHeader, MessageId, FromEmail);
        end;
    end;
    local procedure CreateDraftMail(OauthHeader: Dictionary of[Text, SecretText]; MessageJson: JsonObject; FromEmail: Text): Text var
    ...
    local procedure SendDraftMail(OauthHeader: Dictionary of[Text, SecretText]; MessageId: Text; FromEmail: Text): Text var
        MailHttpClient: HttpClient;
        MailHttpContent: HttpContent;
        MailContentHeaders: HttpHeaders;
        MailRequestHeaders: HttpHeaders;
    ...
    begin
        ...
        if MailHttpResponseMessage.IsBlockedByEnvironment()then Error(EnvironmentBlocksErr)end;
    ...
    local procedure GetAttachmentSize(AttachmentJson: JsonObject): Integer var
        JToken: JsonToken;
    begin
    ...
    local procedure GetUploadUrl(AttachmentHttpResponseMessage: HttpResponseMessage): Text var
        ResponseJson: JsonObject;
        JToken: JsonToken;
        ResponseJsonText: Text;
    begin
    ...
    local procedure GetAttachmentContent(var AttachmentJson: JsonObject): Text var
        JToken: JsonToken;
    ...
    local procedure MaximumRequestSizeInBytes(): Integer begin
        exit(4194304); // 4 mb
    end;
    local procedure MaximumAttachmentSizeInBytes(): Integer begin
        exit(3145728); // 3 mb
    end;
    procedure DefaultEmailRateLimit(): Integer begin
        exit(30);
    end;
    local procedure GetHttpErrorMessageAsText(MailHttpResponseMessage: HttpResponseMessage): Text var
        SendEmailErr: Label 'Could not send the email message. Try again later.';
        ErrorMessage: Text;
    begin
